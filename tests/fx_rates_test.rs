...
#[tokio::test]
async fn fx_rates_returns_usd_as_identity_rate() {
    let app = common::setup_test_app().await.expect("setup failed");
    common::create_test_user(&app.state, "alice_fx_a3", "pw")
        .await
        .expect("create user");
    let cookie = common::login_user(&app.router, "alice_fx_a3", "pw")
        .await
        .expect("login user");

    let request = Request::builder()
        .uri("/fx/rates?from=2026-04-01&to=2026-04-02&quotes=USD")
        .method("GET")
        .header("cookie", cookie)
        .body(Body::empty())
        .expect("build request");
    ...
    assert_eq!(rates[1]["date"], "2026-04-02");
