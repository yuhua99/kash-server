...
async fn create_record_tool(
    ...
    let payload = CreateRecordPayload {
        ...
        amount: input.amount,
        currency: DEFAULT_MAIN_CURRENCY.to_string(),
...
async fn edit_record_tool(
    ...
    let affected_rows = conn
        .execute(
            "UPDATE records SET name = ?, amount = ?, currency = ?, category_id = ?, date = ? WHERE id = ? AND owner_user_id = ?",
            (
                updated_name.as_str(),
                updated_amount,
                existing.currency.as_str(),
...
async fn list_records_tool(
    ...
    let mut rows = conn
        .query(
            "SELECT id, name, amount, currency, category_id, date FROM records \
             WHERE owner_user_id = ? \
             AND date BETWEEN ? AND ? \
             AND (? = '' OR category_id = ?) \
             AND (? = '' OR INSTR(LOWER(name), LOWER(?)) > 0) \
...
async fn fetch_record_by_exact_name(
    ...
    let mut rows = conn
        .query(
            "SELECT id, name, amount, currency, category_id, date FROM records WHERE LOWER(name) = LOWER(?) AND owner_user_id = ? ORDER BY date DESC LIMIT 3",
            (trimmed, user_id),
        )
        .await
